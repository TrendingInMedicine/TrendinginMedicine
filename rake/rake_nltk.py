  ...
  def _calculate_word_scores(self, phrase_list):
    ...
    for phrase in phrase_list:
      ...
      for x in range(len(phrase)-1):
          ...
          print(nltk.tag.pos_tag(phrase))
          if(not nltk.tag.pos_tag([phrase[x]])[0][1].startswith('N')):
              degree = degree - 1
          if(nltk.tag.pos_tag([phrase[x]])[0][1].startswith('J') or nltk.tag.pos_tag([phrase[x]])[0][1].startswith('IN')):
              degree = degree + 1
          if(nltk.tag.pos_tag([phrase[x]])[0][1].startswith('R')):
              degree = degree - 1
          else:
              degree = degree + 2
